 ...
 * Service de gestion des données étudiants
 * Migré depuis studentProductionDataService.js
 */

import { getSupabaseClient } from '../supabase/client.js';

/**
 * Utilitaires de calcul de moyennes (à migrer vers @edutrack/utils plus tard)
 * TODO: Utiliser gradeCalculator de @edutrack/utils
 */
const computeSubjectAverage = (grades) => {
  if (!grades || grades.length === 0) return 0;
  const sum = grades.reduce((acc, g) => acc + (Number(g.grade) || 0), 0);
  return sum / grades.length;
};

const computeOverallAverage = (subjects, trimester) => {
  if (!subjects || subjects.length === 0) return 0;
  let totalPoints = 0;
  let totalCoef = 0;
  subjects.forEach(s => {
    const coef = Number(s.coefficient) || 1;
    totalPoints += (Number(s.average) || 0) * coef;
    totalCoef += coef;
  });
...
/**
 * Service de données pour les étudiants
 */
export const studentService = {
   ...
   * @param {string} studentId - ID de l'utilisateur étudiant
   */
   ...
   * @param {string} studentId - ID de l'utilisateur (optionnel)
  ...
  async getStudentProfile(studentId) {
    try {
      ...
      if (studentId) {
        this.setUserContext(studentId);
      }
      this.ensureContext();

      console.log('👤 Récupération profil pour user_id:', this.currentStudentId);
  ...
  async getStudentStats(studentId) {
    try {
      ...
      console.log('📊 Récupération stats pour étudiant:', this.currentStudentId);
      ...
      const { data: gradesData, error: gradesError } = await supabase
        .from('grades')
        .select('grade, subject, max_grade, coefficient')
      ...
      try {
        if (gradesData && gradesData.length > 0) {
          ...
          const subjectsArray = Object.keys(subjMap).map(name => ({
            ...
            coefficient: (subjMap[name].grades.reduce((acc, x) =>
              acc + (Number(x.coefficient) || 1), 0) / Math.max(1, subjMap[name].grades.length))
          }));
  ...
  async getStudentGrades(studentId) {
    try {
      ...
      console.log('📝 Récupération notes pour étudiant:', this.currentStudentId);
  ...
  async getStudentAttendance(studentId) {
    try {
      ...
      console.log('📅 Récupération présences pour étudiant:', this.currentStudentId);
  ...
  async getStudentAssignments(studentId) {
    try {
      ...
      console.log('📚 Récupération devoirs pour étudiant:', this.currentStudentId);
  ...
  async getStudentNotifications(studentId) {
    try {
      ...
      console.log('🔔 Récupération notifications pour étudiant:', this.currentStudentId);
  ...
  async getStudentAchievements(studentId) {
    try {
      ...
      console.log('🏆 Récupération achievements pour étudiant:', this.currentStudentId);
  ...
  async getStudentBehavior(studentId) {
    try {
      ...
      console.log('🎯 Récupération comportement pour étudiant:', this.currentStudentId);
  ...
  async getStudentSchedule(studentId) {
    try {
      ...
      console.log('📆 Récupération emploi du temps pour étudiant:', this.currentStudentId);
