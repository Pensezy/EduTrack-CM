-- ====================================
-- MIGRATION INITIALE EDUTRACK CM
-- ====================================
-- Cette migration configure le trigger automatique
-- pour la création de comptes directeurs
-- 
-- IMPORTANT: Ce script doit être exécuté APRÈS avoir créé
-- toutes les tables avec MIGRATION_COMPLETE_22_TABLES.sql
-- ====================================
...
-- 2. Créer la fonction trigger avec permissions élevées
...
DECLARE 
    ...
    next_year text;
BEGIN
    -- 1. Insérer dans users (pour TOUS les rôles)
    ...
    -- 2. Si c'est un directeur, créer son école + données par défaut
    IF COALESCE(NEW.raw_user_meta_data->>'role', 'student') = 'principal' THEN
        ...
        IF school_data IS NOT NULL THEN
            ...
            -- 2.2 Lier l'utilisateur à l'école
            UPDATE public.users 
            SET current_school_id = school_uuid 
            WHERE id = NEW.id;
            
            -- 2.3 Créer l'année académique
            ...
            -- 2.7 Périodes d'évaluation (Trimestres ou Semestres selon type d'école)
            IF COALESCE((school_data->>'type')::text, 'public') IN ('primaire', 'college', 'college_lycee') THEN
                -- Trimestres pour primaire/collège
...
-- 4. Désactiver RLS (pour éviter les problèmes de permissions en développement)
...
BEGIN
    RAISE NOTICE '==========================================';
    RAISE NOTICE 'TRIGGER AUTOMATIQUE INSTALLE AVEC SUCCES';
    RAISE NOTICE '==========================================';
    ...
    RAISE NOTICE 'RLS: DESACTIVE sur 8 tables';
    RAISE NOTICE '==========================================';
    RAISE NOTICE 'Configuration automatique lors creation compte:';
    RAISE NOTICE '- Annee academique (annee en cours)';
    RAISE NOTICE '- 5 types de notes';
    RAISE NOTICE '- 4 types de presence';
    RAISE NOTICE '- 6 types de paiement';
    RAISE NOTICE '- Periodes evaluation (Trimestres/Semestres)';
    RAISE NOTICE '- 6 roles utilisateur';
    RAISE NOTICE '==========================================';
    RAISE NOTICE 'PRET A CREER DES COMPTES DIRECTEURS !';
    RAISE NOTICE '==========================================';
END $$;
